...
import { APP_DEFAULT_ROLE, detectMobileScreen, getJWTPayload } from '@helpers'
...
const Index = ({ children }) => {
  ...
  useEffect(() => {
    const isAuthRoutes: boolean = /^(\/(login|register|password)\/?\w*)/g.test(pathname || '')
  ...
  useEffect(() => {
    ...
    const countDown = setInterval(() => {
      ...
      // const remainingSeconds = moment.unix(exp).subtract(6, 'h').add(60, 's').diff(moment(), 's')
