...
export const randomStringV2 = (length: number = 64, options: RandomStringOptions = {}): string => {
...
export const isDev: boolean = process && process.env && process.env.NODE_ENV === 'development'
...
export const formatCurrencyShort = (value: number): string => {
  ...
  for (const unit of units) {
    if (absValue >= unit.value) {
      const short = (value / unit.value).toFixed(2)
      return `Rp. ${parseFloat(short)}${unit.suffix}`
    }
  }
